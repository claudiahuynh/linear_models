...
Fit three models. 
gam() would give you a smooth fit.
s(range) is analogous to using polynomials but polynomials are numerically. Collection of smooth function that captures non-linearity. 

s(range, k = 30) uses more functions than default. sp = 10e-6 (smoothing parameter) is set arbitrarily small which will overfit 
```{r}
...
Fit linear models to all 100 training datasets. Extract RMSE. DO the same thing for swiggly and smooth models
...
Look at models
Linear model looks ok but not capturing the head and tail part of the data. 
```{r}
child_df |>  
  add_predictions(linear_model) |> 
  ggplot(aes(x = weight, y = armc)) +
